...
interface WorkspaceContextValue {
  ...
  selfAssignTask: (taskId: string, userId?: string) => Promise<void>;
...
export function WorkspaceProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const snapshot = await workspaceApi.bootstrap();
  ...
  const addMember = useCallback(async (name: string, email: string) => {
    if (!team) throw new Error('Crie uma equipe antes de adicionar membros.');

    await runAction(async () => {
      const member = await workspaceApi.addMember(team.id, { name, email });
      setMembers((current) => [...current, member]);
      setTeam((current) => (current ? { ...current, members: [...current.members, member] } : current));
  ...
  const removeMember = useCallback(async (memberId: string) => {
    ...
    await runAction(async () => {
      await workspaceApi.removeMember(team.id, memberId);
      setMembers((current) => current.filter((member) => member.id !== memberId));
      setTasks((current) => current.map((task) => (task.assigneeId === memberId ? { ...task, assigneeId: undefined } : task)));
  ...
  const transferLeadership = useCallback(async (memberId: string) => {
    ...
    await runAction(async () => {
      await workspaceApi.transferLeadership(team.id, memberId);
      setMembers((current) =>
        current.map((member) => ({
          ...member,
    ...
    }, 'Liderança transferida com sucesso.');
  }, [runAction, team]);
  ...
  const rejectTask = useCallback(async (taskId: string) => {
    ...
    }, 'Tarefa reprovada e devolvida para ToDo.');
  }, [runAction]);
