...
export function TasksPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Tarefas"
        description="Quadro principal consumindo as transições reais do backend."
        action={
      ...
      {!project && !isLoading && (
        <div className="rounded-2xl border border-dashed bg-white p-6 text-sm text-slate-600 shadow-soft">
          Crie um projeto para começar a registrar tarefas reais no backend.
        </div>
      )}
                ...
                <CardContent className="space-y-3">
                  {columnTasks.map((task) => (
                    <TaskCard
                      ...
                      canReview={isLeader}
                      onSelfAssign={(taskId) => void selfAssignTask(taskId, user?.id ?? '')}
                      onPause={(taskId) => void pauseTask(taskId)}
                      onResume={(taskId) => void resumeTask(taskId, user?.id ?? '')}
